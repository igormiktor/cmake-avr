...
# Matthias Kleemann
#
# Modfied by Igor Mikolic-Torreira
#
##########################################################################
...
##########################################################################
set( CMAKE_CXX_FLAGS "-fno-exceptions"  CACHE STRING "Default C++ flags for all builds" )

set( CMAKE_C_FLAGS_RELEASE "-O3" CACHE STRING "Default C flags for release" )
set( CMAKE_CXX_FLAGS_RELEASE "-O3" CACHE STRING "Default C++ flags for release" )

set( CMAKE_C_FLAGS_MINSIZEREL "-Os -mcall-prologues" CACHE STRING "Default C flags for minimum size release" )
set( CMAKE_CXX_FLAGS_MINSIZEREL "-Os -mcall-prologues" CACHE STRING "Default C++ flags for minimum size release" )

set( CMAKE_C_FLAGS_DEBUG "-g" CACHE STRING "Default C flags for debug" )
set( CMAKE_CXX_FLAGS_DEBUG "-g" CACHE STRING "Default C++ flags for debug" )

set( CMAKE_C_FLAGS_RELWITHDEBINFO "-O3 -g" CACHE STRING "Default C flags for release with debug info" )
set( CMAKE_CXX_FLAGS_RELWITHDEBINFO "-O3 -g" CACHE STRING "Default C++ flags for release with debug info" )
...
function( add_avr_executable EXECUTABLE_NAME )
   ...
   add_custom_command(
      ...
      COMMAND
         ${AVR_SIZE_TOOL} ${AVR_SIZE_ARGS} ${elf_file} "|" ${AWK} ${AWK_ARGS}
      DEPENDS ${elf_file} firmwaresize.awk
   )
   ...
   add_custom_target(
      firmwaresize.awk
      COMMAND
        echo "BEGIN {ORS=\"\";print \"\\\\n\\\\033[1;33mFirmware size (\"}" > firmwaresize.awk
      COMMAND
        echo "/^Device/ {print \$2 \") is...  \"}" >> firmwaresize.awk
      COMMAND
        echo "/^Program/ {print \"Flash (program): \" \$2 \" \" \$3 \" \" \$4 \")  \"}" >> firmwaresize.awk 
      COMMAND
        echo "/^Data/ {print \"RAM\ (globals): \" \$2 \" \" \$3 \" \" \$4 \")  \"}" >> firmwaresize.awk
      COMMAND
